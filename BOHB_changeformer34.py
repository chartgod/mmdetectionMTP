...
# 로그 파일 경로 설정
all_trials_log_file_path = '/home/lsh/share/CD/open-cd/tools/hpbandster_trials_log34.txt'
...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        config_path = '/home/lsh/share/CD/open-cd/configs/changeformer/changeformer_mit-b0_256x256_40k_levircd.py'
        ...
        modified_config_path = '/home/lsh/share/CD/open-cd/tools/hpbandster_temp_config.py'
        cfg.dump(modified_config_path)

        # 학습 로그를 파일로 저장하기 위해 로그 경로 지정
        log_file_path = f'/home/lsh/share/CD/open-cd/tools/hpbandster_trial_log34.txt'

        # 학습 시작
        try:
            with open(log_file_path, 'w') as log_file:
                # GPU 3, 4번에 할당되도록 환경 변수 설정
                env = os.environ.copy()
                env["CUDA_VISIBLE_DEVICES"] = "3,4"  # GPU 3, 4번 사용

                # 학습 프로세스 실행
                subprocess.run(
                    ['python', '/home/lsh/share/CD/open-cd/tools/train1_2.py', modified_config_path],
...
NS = hpns.NameServer(run_id='bohb', host='localhost', port=9092)
NS.start()

worker = MyWorker(nameserver='localhost', run_id='bohb', nameserver_port=9092, timeout=120)
